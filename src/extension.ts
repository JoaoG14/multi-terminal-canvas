...
function getAvailableShells(): { name: string, path: string }[] {
  ...
  if (os.platform() === 'win32') {
    ...
    const ps5 = path.join(process.env.SystemRoot || 'C:\\Windows',
      'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe');
    if (fs.existsSync(ps5)) shells.push({ name: 'PowerShell 5', path: ps5 });
    ...
    for (const p of [
      ...
      if (fs.existsSync(p)) { shells.push({ name: 'PowerShell 7', path: p }); break; }
    }
    ...
    for (const p of [
      ...
      if (fs.existsSync(p)) { shells.push({ name: 'Git Bash', path: p }); break; }
    }

    // WSL
    const wsl = path.join(process.env.SystemRoot || 'C:\\Windows', 'System32', 'wsl.exe');
    if (fs.existsSync(wsl)) shells.push({ name: 'WSL', path: wsl });
    ...
    for (const p of candidates) {
      ...
      if (!seen.has(name) && fs.existsSync(p)) { seen.add(name); shells.push({ name, path: p }); }
    }
  }
